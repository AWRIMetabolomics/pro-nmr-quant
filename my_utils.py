...
def get_df_auc(query_df, df_dict, results_dict, multiplets_ls, floor_intensities_bool=False):
    """get scaling factor and store in a df. Run after do_1d_std_search(). 
    ...
    floor_intensities_bool: bool; whether or not to set all negative intensities to 0. 
    ...
    for k in list(results_dict.keys()):
        ...
        for multiplet_i in list(results_dict[k].keys()):
            ...
            if floor_intensities_bool:
                intensity_arr = dt_query["intensity"].values - min(dt_query["intensity"].values)
                dt_query["intensity"] = intensity_arr
                intensity_arr = dt_target["intensity"].values - min(dt_target["intensity"].values)
                dt_target["intensity"] = intensity_arr
...
def adjust_to_ref_peak(df, ref_coords, tolerance_window, ref_peak_target_height=1E6):
    ...
    ref_peak_target_height: int; ref peak target height. Arbitrarily set to 1E6 by default.
...
def do_2d_std_search(query_df, target_df, multiplets_ls, normxcorr2_mode="same", search_region_padding_size=0.01):
    """
...
def viz_enema(query_df,
    ...
    if len(results_dict.keys()) > 1:
        ...
        for i in range(len(results_dict.keys())):
            ...
            subtitle += f"@{round(match_centre_ppm, 4)}\ns.f.={round(results_dict[k]['scaling_factor_1d'], 3)}"
            ...
            # Draw all subtitles
            axarr[0, 0].set_ylabel("multiplet\n1D un-scaled", fontsize=title_fontsize)
            axarr[1, 0].set_ylabel("multiplet\n1D scaled", fontsize=title_fontsize)
...
def get_html_contents(output_svg, svg_resize_coeff = 0.4):
    """
    Prepare a list of str, in html format, for writing out. 
    
    PARAMS
    ------
    output_svg: ls of str; svg from matplotlib, already split by "\n".
    svg_resize_coeff: multiplicative coefficient to resize svg height and width by. 
    ...
    for i in range(len(output_svg)):
        if "<svg height" in output_svg[i]:
            pattern = re.compile(r'<svg height="\d{1,11}pt"')
